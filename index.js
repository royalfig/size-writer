...
async function createPromptCache(url, selector, imageSizes) {
  try {
    await fs.writeFile(
      'cache.json',
      JSON.stringify({ url, selector, imageSizes })
    );
    cachePrompts = { url, selector, imageSizes };
  } catch (error) {
    console.log(error);
  }
}
...
let schema = {
  properties: {
    url: {
      description: 'Enter the URL of the webpage',
      required: true,
      type: 'string',
    },
    selector: {
      description: 'Enter the CSS selector of the image',
      required: true,
      type: 'string',
    },
    imageSizes: {
      description: 'Enter available image sizes',
      required: false,
      type: 'string',
...
async function readCache() {
  try {
    const res = await fs.readFile('cache.json', 'utf8');
    cachePrompts = JSON.parse(res);
  } catch (error) {
    console.info('No cache detected. Starting prompts...');
  }
}
...
async function parseSizes() {
  ...
  if (!cachePrompts) {
    ...
    prompt.get(schema, async function (err, result) {
      ...
      await createPromptCache(result.url, result.selector, result.imageSizes);

      const imageSizesAsNum = createNumberArray(result.imageSizes);
      ...
      console.log('🚀 ~ file: index.js:82 ~ sizes:', sizes);
    });
  } else {
    const sizes = await generateSizesAttribute(
      cachePrompts?.url,
      cachePrompts?.selector,
      createNumberArray(cachePrompts?.imageSizes),
      viewports
    );
    console.log('🚀 ~ file: index.js:92 ~ parseSizes ~ sizes:', sizes);
  }
}
